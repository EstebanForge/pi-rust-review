...
Narrow scope with `path` (a file or directory). The tool runs `git` **from** that path — so `path` can point into a nested repo (e.g. a crate inside a workspace whose root is not itself a git repo).
